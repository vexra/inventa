...
import { getUnitAssets } from './actions'
...
export default async function UnitAssetsPage({ searchParams }: PageProps) {
  ...
  let assetResult: Awaited<ReturnType<typeof getUnitAssets>> | null = null
  ...
  if (!userUnitId) {
    ...
    assetResult = await getUnitAssets(
      page,
      limit,
      query,
      statusFilter,
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Aset Unit</h1>
          <p className="text-muted-foreground">
            Kelola dan pantau aset di seluruh ruangan unit Anda.
          </p>
        </div>
      </div>
