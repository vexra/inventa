...
export const auth = betterAuth({
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url }) => {
      await sendEmail({
        to: user.email,
        subject: 'Aktivasi Akun - Inventa FMIPA Unila',
        html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="background-color: #f3f4f6; padding: 20px; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #333;">
        <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
          ...
          <div style="padding: 30px 20px;">
            <h2 style="margin-top: 0; font-size: 18px; color: #1f2937;">Aktivasi Akun Pengguna</h2>
            <p style="line-height: 1.6; color: #4b5563;">Halo <strong>${user.name || 'Pengguna'}</strong>,</p>
            
            <p style="line-height: 1.6; color: #4b5563;">
              Akun Anda telah didaftarkan ke dalam sistem Inventa. Untuk mengaktifkan akun dan mulai menggunakan aplikasi, silakan verifikasi alamat email Anda dengan mengklik tombol di bawah ini.
            </p>
            
            <div style="text-align: center; margin: 30px 0;">
              <a href="${url}" style="background-color: #2563EB; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Verifikasi & Aktifkan Akun</a>
            </div>
